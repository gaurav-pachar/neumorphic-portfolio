const ExperienceData = {
    WorkEx : {
        Name: "Work Experience",
        list: [
            {
                organisation: "Blue Compute",
                timePeriod: "Aug 2021 - Present",
                projectDetails: [
                    "Build a Next.js SaaS Application from scratch, using libraries like Redux, AG-Grid, AWS, etc.",
                    "Worked on a SaaS Application that used Reactjs and Redux for frontend, with serverless backend deployed on AWS lambda and used DynamoDB for Database services.",
                    "Developed a Next.js app from scratch, it was a part of the above-mentioned SaaS application and was using the same serverless backend, deployed on AWS lambda.",
                    "Developed a single-page VueJs application.",
                    "Worked on multiple NextJs websites using stackbit platform.",
                    "Developed a GatsbyJS website.",
                    "Build a ChatBot using Bot Framework Composer."
                ] 
            }
        ]
    },
    OpenSource: {
        Name: "Open Source Projects",
        list: [
            {
                name: "Lead Management Application",
                details: "A application which help user to manage lead in a business, this one is a example for House Plumbings and Electricals Contractor",
                image: "url",
                projectUrl: "https://github.com/gaurav-pachar/lead-management-application"
            },
            {
                name: "Neumorphic Portfolio",
                details: "Neumorphism Desgin Based Web Developer Portfolio",
                image: "url",
                projectUrl: "https://github.com/gaurav-pachar/neumorphic-portfolio"
            }
        ]
    }
}
export default ExperienceData;

